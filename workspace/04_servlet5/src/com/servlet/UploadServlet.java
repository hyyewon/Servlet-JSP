...
public class UploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 파일 업로드 기능 구현
		/*
		 * enctype="multipart/form-data" 지정되었으면 더 이상 request.getParameter(key) 사용 불가
		 * request.setCharacterEncoding("utf-8") 사용불가
		 */

		// **https://commons.apache.org/proper/commons-fileupload/using.html -> The
		// simplest case
...
//			List<DiskFileItem> items = upload.parseRequest(request);
			...
			// **Processing 내용에서 iterator로 수정
			Iterator<FileItem> iter = items.iterator();
			// Process the uploaded items
			while (iter.hasNext()) {
				FileItem item = iter.next();
				if (item.isFormField()) {
					// type="file" 아닌것 처리 ==> type="text" 처리
